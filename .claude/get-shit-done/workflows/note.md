...
- **Global scope**: `C:/dev/Antigravity/ORKI/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/dev/Antigravity/ORKI/.claude/notes/`)
...
2. Glob `C:/dev/Antigravity/ORKI/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/dev/Antigravity/ORKI/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/dev/Antigravity/ORKI/.claude/notes/` — works in any directory
